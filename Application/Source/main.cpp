...
#define GIF_RECORDING // TODO: Either refactor all of this out or finish implementing this flag
...
namespace ls {
    ...
    } captureInfo; // TODO: Release resources we allocated for this!
    ...
    void RecordFrameCaptureCmds() {
      ...
      CmdImageBarrier( captureInfo.captureFrameCmds,
                       ...
                       vk::PipelineStageFlags(),             // wait for nothing, VALIDATION: fails because this is 0
    ...
    void StopCapturing() {
      ...
      while ( captureInfo.captureFrameCount ) {
        ...
        if ( !readResult || (bytesRead != captureInfo.capturedFrameImage.size) ) {
          std::cout << "ReadFile failed:" << GetLastError() << std::endl; // FIXME: Somehow not all frames are read back!
          LsError();
        }
    ...
    void CreateInstance() {
        ...
        for( size_t i = 0; i < INSTANCE_EXTENSIONS.size(); ++i ) {
            if( !CheckExtensionAvailability( INSTANCE_EXTENSIONS[i], available_instance_extensions ) ) {
                LsAbort(std::string("Could not find instance extension named \"") + 
                        std::string(INSTANCE_EXTENSIONS[i]) + 
                        std::string("\"!"));
            }
        }

        vk::ApplicationInfo appliactionInfo(LS_PRODUCT_NAME,
                                            LS_VERSION,
                                            NULL,
                                            NULL,
                                            VK_API_VERSION_1_0);
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  try {
    ...
    ls::CreateMainWindow();
    LsSetDialogParentWindow(ls::windowHandle);
